...
import { z } from "zod";
import { withValidatedParams } from "@/lib/validation";
...
export default withValidatedParams(
  { paramsSchema },
  async ({ params }) => {
  const messages = await backend.conversationFacade.findMessagesByConversationId(params.id) ?? [];
